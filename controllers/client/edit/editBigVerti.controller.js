...
function getLatestOriginalPhotos(limit = 4) {
    ...
    return fs
        ...
        .sort((a, b) => {
           ...
           // 🔴 SỬA LẠI: numB - numA để số LỚN NHẤT (ảnh mới nhất) lên đầu
            return numB - numA; 
        })
        // 🔴 SỬA LẠI: Lấy 'limit' phần tử ĐẦU TIÊN của danh sách đã đảo ngược
        .slice(0, limit);
...
module.exports.postEditBigVerti = async (req, res) => {
    ...
    photos.map((photo) => {
        ...
        if (!fs.existsSync(output)) {
            ...
            return applyFilterFunction(input, output, filterName);
...
module.exports.editBigVerti = (req, res) => {
