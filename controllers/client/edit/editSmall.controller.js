...
const { mergeSmall } = require("../../../helper/merge-small.js");
const { filters } = require("../../../helper/filters-list.js");
const sharp = require("sharp");

// ================== CONFIG IN ẢNH ==================
const PRINT_WIDTH = 600;
...
module.exports.postEditSmall = async (req, res) => {
    try {
        ...
        // ====== MERGE ======
        const outputFileName = await mergeSmall(
            sourceDir,
            selectedFrame,
            frameType,
            PRINT_WIDTH,
        ...
        const filterEnd = Date.now(); // ⏱ kết thúc
    console.log(
        `🎨 Apply filter "${filterName}" mất ${(filterEnd - filterStart) / 1000}s`
    );
        return res.json({
            success: true,
            cached: false,
        ...
        console.error("❌ postEditSmal error:", err);
        res.status(500).json({ success: false });
    }
};

// ================== RENDER EDIT PAGE ==================
module.exports.editSmall = (req, res) => {
