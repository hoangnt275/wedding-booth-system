...
async function mergeSummer(
    ...
    async function resizeFrameToRaw(input, w, h) {
        return sharp(input)
    ...
    async function resizeToRaw(input, w, h) {
        return sharp(input)
            .rotate(-90) // 👈 THÊM DÒNG NÀY VÀO ĐÂY
            .resize(w, h, { fit: "cover", position: "centre" })
            .raw()
            .toBuffer({ resolveWithObject: true });
    }
    ...
    const [rawFrame, ...rawPhotos] = await Promise.all([
        resizeFrameToRaw(framePath, PRINT_WIDTH, PRINT_HEIGHT),
        ...photoFiles.map((f) =>
    ...
    const layers = [
        ...
        {
            input: rawPhotos[3].data,
            raw: rawPhotos[3].info,
            top: START_Y + (PHOTO_H + GAP_Y)*3,
            left: MARGIN_X,
        },
    ];
